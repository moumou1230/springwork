	...
	SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		...
		http// Security가 적용될 URI
				.authorizeHttpRequests((authrize) -> authrize.dispatcherTypeMatchers(DispatcherType.FORWARD).permitAll()
						// permitAll은 인증받지 못한 모든사람 포함 //경로에 권한이 붙는다.
						.requestMatchers("/", "/all").permitAll()// 경로(한번에 여러개의 경로에대해서 권한등록 가능),requestMatchers url패턴
		...
		http.csrf(csrf->csrf.disable()); //csrf 일시적으로 안되게. csrf 돌아가면 put ,post등 csrf 설정안된곳은 실행이 안된다.
		return http.build();
	}
